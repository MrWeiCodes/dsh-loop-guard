...
- **五层检测，各管一种形态**：纯推理无输出、复述上一步材料、单次调用内低熵重复、推理内部的**周期循环**、以及推理内部的**短语池重排**。后两条是互补的，见下。
...
- **精确判据而非低熵比率**：真实数据里 `repeatRatio` 最高的非循环调用是 **0.833**，但它**完全没有周期**——比率规则会误伤它，精确规则不会。
...
不会。判定用的是**逐字周期**，不是时长、也不是低熵比率。实测 997 个有产出的真实调用零误报；生成的表格、日志、CSS、JSON 这些「合理重复」的输出也都不会被判为循环。可见输出侧的周期规则在 **2,973 条**真实长文本（≥1500 字符，跨多个工作区）上标定，周期上限从 64 试到 4096，**误报 0 次**。
